...
chatSchema.methods.addMessage = function(senderId, content, messageType = 'text', fileUrl = '', fileName = '') {
  const message = {
    sender: senderId,
    content: content,
...
chatSchema.statics.findOrCreateChat = async function(user1Id, user2Id) {
  ...
  if (!chat) {
    ...
    await chat.save();
    chat = await chat.populate('participants', 'username firstName lastName avatar');
  }
