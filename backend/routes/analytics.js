...
// @access  Private
router.get('/collaboration', protect, [
  query('timeRange')
    .optional()
  ...
  try {
    ...
    const forkStats = await Post.aggregate([
      {
        $match: {
          $or: [
            ...
            { forkedFrom: { $exists: true }, author: userId, createdAt: { $gte: startDate } }
          ]
        }
      },
      {
    ...
    const totalActivities = (reviewStats[0]?.totalReviews || 0) + (forkStats[0]?.totalForks || 0);
    const collaborationScore = Math.min(100, Math.max(0, totalActivities * 10));
    ...
    // Monthly activity data (mock data for now)
