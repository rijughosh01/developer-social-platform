...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!post) {
      return res
        .status(404)
        .json({ success: false, message: "Review request not found" });
    }
...
router.put(
  "/review-requests/:reviewId/requester-reply",
  protect,
  asyncHandler(async (req, res) => {
...
router.post(
  ...
  protect,
  asyncHandler(async (req, res) => {
    const { title, description, code, codeLanguage, difficulty, tags } =
      req.body;
    const notificationService = new NotificationService(req.app.get("io"));
    ...
    if (originalPost.type !== "code") {
      return res
        .status(400)
        .json({ success: false, message: "Only code posts can be forked" });
    }
    ...
    const forkedCount = await Post.countDocuments({
      forkedFrom: originalPost._id,
    });
    ...
    if (
      originalPost.author &&
      originalPost.author._id.toString() !== req.user._id.toString()
    ) {
      await notificationService.createNotification({
        recipient: originalPost.author._id,
        sender: req.user._id,
        type: "fork_created",
        title: `${forkedPost.author.firstName || ""} ${
          forkedPost.author.lastName || ""
        } forked your code post`.trim(),
  ...
  asyncHandler(async (req, res) => {
    ...
    if (post.type !== "code") {
      return res
        ...
        .json({
          success: false,
          message: "Only code posts can have review requests",
        });
    }
    ...
    if (post.author && post.author._id.toString() !== req.user._id.toString()) {
      await notificationService.createNotification({
        ...
        title: `${req.user.firstName || ""} ${
          req.user.lastName || ""
        } requested a review on your code post`.trim(),
