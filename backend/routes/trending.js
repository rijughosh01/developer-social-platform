...
// @desc    Get trending posts, projects, and developers
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const posts = postsRaw.map(post => {
    ...
    if (obj.author && post.author && post.author.followersCount !== undefined) {
  ...
  const projects = projectsRaw.map(project => {
    ...
    if (obj.owner && project.owner && project.owner.followersCount !== undefined) {
  ...
  const developersRaw = await User.find()
    ...
    .limit(5)
    .select('username firstName lastName avatar followersCount bio followers');

  const developers = developersRaw.map(dev => dev.toObject({ virtuals: true }));
