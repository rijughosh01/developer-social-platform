...
class NotificationService {
  ...
  async createNotification(data) {
    try {
      ...
      console.log('✅ createNotification: Notification created in database:', notification._id);
      
      // Populate sender info for real-time emission
      await notification.populate('sender', 'username firstName lastName avatar');
      console.log('✅ createNotification: Sender populated:', notification.sender);
      
      // Send real-time notification to recipient if IO is available
      if (this.io) {
        console.log('🔔 createNotification: Sending real-time notification to:', data.recipient.toString());
        ...
        console.log('⚠️ createNotification: No IO instance available for real-time notification');
      }
  ...
  async createMessageNotification(senderId, recipientId, chatId, messageId, messageContent) {
  ...
  async createLikePostNotification(likerId, postAuthorId, postId, postTitle) {
    const liker = await User.findById(likerId).select('username firstName lastName');
  ...
  async createLikeProjectNotification(likerId, projectOwnerId, projectId, projectTitle) {
    const liker = await User.findById(likerId).select('username firstName lastName');
  ...
  async createCommentPostNotification(commenterId, postAuthorId, postId, postTitle, commentContent) {
    ...
    try {
      const commenter = await User.findById(commenterId).select('username firstName lastName');
      ...
      console.log('✅ NotificationService: Comment notification created successfully:', notification._id);
      return notification;
    } catch (error) {
  ...
  async createCommentProjectNotification(commenterId, projectOwnerId, projectId, projectTitle, commentContent) {
    const commenter = await User.findById(commenterId).select('username firstName lastName');
  ...
  async createFollowNotification(followerId, followedId) {
    const follower = await User.findById(followerId).select('username firstName lastName avatar');
  ...
  // Unfollow notification (optional - some apps don't send these)
  async createUnfollowNotification(followerId, followedId) {
    const follower = await User.findById(followerId).select('username firstName lastName');
  ...
  async createMentionNotification(mentionerId, mentionedUserId, postId, postTitle, mentionContent) {
    const mentioner = await User.findById(mentionerId).select('username firstName lastName');
    ...
    return await this.createNotification({
      ...
      message: mentionContent.length > 50 ? `${mentionContent.substring(0, 50)}...` : mentionContent,
      data: {
        postId,
        url: `/posts/${postId}`
      }
  ...
  async createProjectInviteNotification(inviterId, inviteeId, projectId, projectTitle) {
    const inviter = await User.findById(inviterId).select('username firstName lastName');
  ...
  async createCollaborationRequestNotification(requesterId, projectOwnerId, projectId, projectTitle) {
  ...
  async createSystemNotification(recipientId, title, message, data = {}) {
    return await this.createNotification({
      recipient: recipientId,
      sender: recipientId, // System notifications don't have a sender
      type: 'system',
      title,
      message,
      data
  ...
  // Bulk notifications (for system announcements, etc.)
  ...
  async updateUnreadCount(userId) {
    ...
    if (this.io) {
      this.io.to(userId.toString()).emit('unread-count-update', { unreadCount });
    }
    return unreadCount;
  }
