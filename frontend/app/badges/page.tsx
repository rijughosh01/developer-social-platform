      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {BADGES.map((badge) => {
          ...
          return (
            <div
              key={badge.key}
              className={`flex items-center gap-4 p-4 rounded-xl shadow-md border ${
                badge.color
              } ${earned ? "opacity-100" : "opacity-50"} transition`}
            >
              ...
              <div>
                <div className="font-semibold text-lg flex items-center gap-2">
                  {badge.label}
                  {earned && (
                    <span className="ml-1 text-green-600" title="Earned">
                      <FiUserCheck />
                    </span>
                  )}
                </div>
