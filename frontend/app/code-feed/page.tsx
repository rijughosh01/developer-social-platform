...
export default function CodeFeedPage() {
      ...
      case 'trending':
        ...
        filteredPosts.sort((a, b) => {
          const aScore = (a.likesCount || 0) + (a.commentsCount || 0) * 2 + 
            Math.max(0, 7 - Math.floor((Date.now() - new Date(a.createdAt).getTime()) / (1000 * 60 * 60 * 24)));
          const bScore = (b.likesCount || 0) + (b.commentsCount || 0) * 2 + 
            Math.max(0, 7 - Math.floor((Date.now() - new Date(b.createdAt).getTime()) / (1000 * 60 * 60 * 24)));
  ...
  const getDifficultyLevel = (post: any) => {
    ...
    if (lines > 50 || hasFunctions && hasLoops && hasConditionals) return 'advanced'
          ...
          <div className="space-y-6">
            ...
            {filteredPosts.length > 0 && (
              <div className="text-sm text-gray-600">
                Showing {filteredPosts.length} code post{filteredPosts.length !== 1 ? 's' : ''}
                {selectedLanguage !== 'all' && ` in ${languageOptions.find(l => l.value === selectedLanguage)?.label}`}
                {selectedDifficulty !== 'all' && ` at ${difficultyOptions.find(d => d.value === selectedDifficulty)?.label}`}
              ...
              {isLoading ? (
                ...
                <div className="bg-white rounded-lg shadow p-8 text-center">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">
                    {searchQuery || selectedLanguage !== 'all' ? 'No matching code posts' : 'No code posts yet'}
                  </h3>
