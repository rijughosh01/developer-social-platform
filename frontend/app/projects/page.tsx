...
export default function ProjectsPage() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    let result = projects.filter((project) => {
      ...
      const matchesMine =
        !onlyMine || user?._id === (project as any).owner?._id;
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-white/90 mt-2 max-w-2xl">
                  Explore community-built projects, find inspiration, and
                  showcase your work.
                </p>
                <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-white/80">
                  <div className="flex items-center gap-2">
                    <FiAward /> Curated selection
                  </div>
                  <div className="flex items-center gap-2">
                    <FiTag /> Filter by category & status
                  </div>
                </div>
              </div>
              <button
          ...
          {loading && (
            ...
            >
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="animate-pulse bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden"
                >
          ...
          {!loading && (
            ...
            >
              {filteredProjects.map((project) => (
                <div
                  key={project._id}
                  className={`relative bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group cursor-pointer transition hover:shadow-lg ${
                    layout === "list" ? "md:flex" : ""
                  }`}
                  onClick={(e) => {
                ...
                >
                  ...
                  {project.image && (
                    <div
                      className={`${
                        layout === "list" ? "md:w-56 md:flex-shrink-0" : ""
                      } w-full h-40 bg-gray-100 overflow-hidden`}
                    >
                      <img
                        src={project.image}
                        alt={project.title}
                    ...
                    <div className="flex items-center justify-between gap-3 mb-2">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-primary-600 flex items-center justify-center overflow-hidden">
                          <img
                            src={getAvatarUrl({
                              firstName: project.owner?.firstName || "User",
                              lastName: project.owner?.lastName || "Name",
                      ...
                      <span
                        className={`inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-full border ${statusBadgeStyle(
                          project.status
                        )}`}
                      >
                        {project.status}
          ...
          {!loading && filteredProjects.length === 0 && (
            <div className="mt-10 flex flex-col items-center justify-center text-center bg-white border border-gray-200 rounded-2xl p-10">
              ...
              <p className="text-gray-600 mb-4 max-w-md">
                Try adjusting search or filters. You can also create a new
                project and be the first to showcase it!
              </p>
              <button
