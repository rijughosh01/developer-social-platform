...
export function CollaborationAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await api.get(`/analytics/collaboration?timeRange=${timeRange}`)
  ...
  const getCollaborationLevel = (score: number) => {
    ...
    if (score >= 30) return { level: 'Beginner', color: 'text-yellow-600', bg: 'bg-yellow-100' }
    return { level: 'New', color: 'text-gray-600', bg: 'bg-gray-100' }
  }

  const getBadgeIcon = (badgeName: string) => {
    switch (badgeName.toLowerCase()) {
      case 'review master': return <FiMessageSquare className="h-6 w-6" />
      case 'fork champion': return <FiGitBranch className="h-6 w-6" />
      case 'quick responder': return <FiClock className="h-6 w-6" />
      case 'team player': return <FiUsers className="h-6 w-6" />
      case 'trending': return <FiTrendingUp className="h-6 w-6" />
      default: return <FiGift className="h-6 w-6" />
    }
  }
  ...
  if (!analytics) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-500">Start collaborating to see your analytics!</p>
      </div>
    )
  }

  const collaborationLevel = getCollaborationLevel(analytics.collaborationScore)

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-900">Collaboration Analytics</h2>
      ...
      <div className="bg-gradient-to-r from-primary-50 to-blue-50 rounded-lg p-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Collaboration Score</h3>
            <p className="text-sm text-gray-600">Your overall collaboration performance</p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold text-primary-600">{analytics.collaborationScore}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <div className="mt-2 text-xs text-gray-500">
            {analytics.completedReviews} completed, {analytics.pendingReviews} pending
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{analytics.totalForks}</p>
            </div>
          </div>
          <div className="mt-2 text-xs text-gray-500">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{analytics.topCollaborators.length}</p>
            </div>
          </div>
          <div className="mt-2 text-xs text-gray-500">
          ...
          {analytics.topCollaborators.length === 0 ? (
            ...
            analytics.topCollaborators.map((collaborator, index) => (
              <div key={collaborator._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  <p className="font-semibold text-gray-900">{collaborator.collaborationCount}</p>
      ...
      {/* Badges */}
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Collaboration Badges</h3>
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg ${
                  badge.earned ? 'bg-primary-600 text-white' : 'bg-gray-300 text-gray-500'
                }`}>
