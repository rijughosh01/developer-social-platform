...
import { FiGitBranch, FiEye, FiUsers, FiTrendingUp, FiClock, FiX } from 'react-icons/fi'
...
export function ForkHistory() {
  ...
  const [filter, setFilter] = useState<'all' | 'my-forks' | 'forks-of-mine'>('all')
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{forkHistory.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {new Set(forkHistory.flatMap(fork => 
                  fork.forks?.map(f => f.author.username) || []
                )).size}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {forkHistory.reduce((sum, fork) => sum + (fork.likesCount || 0) + (fork.copies || 0), 0)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {forkHistory.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No forks found</h3>
          ...
          forkHistory.map((fork) => (
            <div key={fork._id} className="bg-white rounded-lg border border-gray-200 p-6">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${getLanguageColor(fork.codeLanguage)}`}>
                      {fork.codeLanguage}
                    </span>
                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${getDifficultyColor(fork.difficulty)}`}>
                      {fork.difficulty.charAt(0).toUpperCase() + fork.difficulty.slice(1)}
                    </span>
                  </div>
                  ...
                  {fork.forkedFrom && (
                    <div className="bg-blue-50 rounded-lg p-3 mb-3">
                      <p className="text-sm text-blue-800">
                        <span className="font-medium">Forked from:</span> {fork.forkedFrom.title} by {fork.forkedFrom.author.firstName} {fork.forkedFrom.author.lastName}
                      </p>
                    </div>
                  )}
                  ...
                  {fork.forks && fork.forks.length > 0 && (
                    <div className="bg-green-50 rounded-lg p-3 mb-3">
                      <p className="text-sm text-green-800 mb-2">
                        <span className="font-medium">Forked by {fork.forks.length} users:</span>
                      </p>
                      <div className="space-y-1">
                        {fork.forks.slice(0, 3).map((forkItem) => (
                          <p key={forkItem._id} className="text-xs text-green-700">
                            • {forkItem.author.firstName} {forkItem.author.lastName} - {new Date(forkItem.createdAt).toLocaleDateString()}
                          </p>
                        ))}
                        {fork.forks.length > 3 && (
                          <p className="text-xs text-green-700">... and {fork.forks.length - 3} more</p>
                        )}
                      </div>
                    </div>
                  )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold">Fork Code: {selectedFork.title}</h3>
              <button
              ...
              {selectedFork.description && (
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-sm text-gray-700">{selectedFork.description}</p>
                  </div>
                </div>
              )}
