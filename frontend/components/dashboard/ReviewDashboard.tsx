...
import { FiMessageSquare, FiStar, FiClock, FiCheck, FiX, FiEye } from 'react-icons/fi'
...
export function ReviewDashboard() {
  ...
  const handleReviewResponse = async (reviewId: string, status: 'completed' | 'rejected') => {
    ...
    try {
      ...
      toast.success(`Review ${status === 'completed' ? 'completed' : 'rejected'} successfully`)
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default: return <FiMessageSquare className="h-4 w-4" />
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-900">Code Review Dashboard</h2>
              ...
              <div key={review._id} className="bg-white rounded-lg border border-gray-200 p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        <span>Language:</span>
                        <span className="font-medium">{review.post.codeLanguage}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      <span>Requested {new Date(review.createdAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                  ...
                  {review.response && (
                    <div className="bg-blue-50 rounded-lg p-3 mb-2">
                      <div className="text-xs text-blue-700 font-semibold mb-1">Reviewer Response:</div>
                      <div className="text-sm text-blue-900">{review.response}</div>
                    </div>
                  )}
                  {/* Requester Reply */}
                  {review.requesterReply && (
                    <div className="bg-green-50 rounded-lg p-3 mb-2">
                      <div className="text-xs text-green-700 font-semibold mb-1">Your Reply:</div>
                      <div className="text-sm text-green-900">{review.requesterReply}</div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  {review.status === 'pending' && (
                    <div className="flex gap-1">
                      <button
                        onClick={() => handleReviewResponse(review._id, 'completed')}
                        className="px-3 py-1 text-sm bg-green-600 text-white rounded hover:bg-green-700"
                      >
                        <FiCheck className="h-4 w-4" />
                      </button>
                      <button
                        onClick={() => handleReviewResponse(review._id, 'rejected')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                <h4 className="font-medium text-gray-900 mb-2">Code to Review</h4>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => handleReviewResponse(selectedReview._id, 'completed')}
                ...
                <button
                  onClick={() => handleReviewResponse(selectedReview._id, 'rejected')}
                  disabled={isSubmitting || !reviewResponse.trim()}
