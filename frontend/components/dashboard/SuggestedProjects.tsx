...
import { FiFolder, FiStar, FiGitBranch, FiTrash2, FiExternalLink, FiGithub } from 'react-icons/fi'
...
export function SuggestedProjects() {
  ...
  const handleStar = async (projectId: string) => {
    try {
      ...
      setLocalProjects(projects => projects.map(p =>
        p._id === projectId
  ...
  const handleDelete = async (projectId: string) => {
    ...
    try {
      ...
      setLocalProjects(projects => projects.filter(p => p._id !== projectId))
    ...
    <div className="bg-white rounded-lg shadow">
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900">Suggested Projects</h3>
        </div>
      </div>
              ...
              >
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-bold text-gray-900 truncate max-w-[60%]">{project.title}</h4>
                    <div className="flex items-center gap-2">
                      <button
                        className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-semibold border ${project.isLiked ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 'bg-gray-100 text-gray-600 border-gray-200'} hover:bg-yellow-200 hover:text-yellow-800 transition`}
                  ...
                  <p className="text-xs text-gray-500 mb-2 line-clamp-2 min-h-[32px]">{project.description}</p>
      ...
      <ProjectDetailsModal 
        ...
        onProjectUpdate={updated => setLocalProjects(projects => projects.map(p => p._id === updated._id ? { ...p, ...updated } : p))}
      />
    </div>
  )
} 