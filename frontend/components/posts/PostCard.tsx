...
export function PostCard({ post, onUnsave, onPostUpdate }: PostCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 mb-4 border border-gray-100 transition-shadow hover:shadow-lg">
      ...
      {editing ? (
        <form onSubmit={handleEditSave} className="space-y-3 mb-4">
          ...
          <div className="flex gap-2 mt-2">
            <button type="submit" disabled={editLoading} className="px-4 py-2 rounded bg-primary-600 text-white font-semibold hover:bg-primary-700">
              {editLoading ? 'Saving...' : 'Save'}
            </button>
            <button type="button" onClick={() => setEditing(false)} className="px-4 py-2 rounded bg-gray-100 text-gray-700 font-semibold hover:bg-gray-200">
              Cancel
            </button>
          </div>
        </form>
        ...
        <>
          {post.title && (
            <div className="text-xl font-bold text-gray-900 mb-1">{post.title}</div>
          )}
          {post.tags && post.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-2">
              {post.tags.map((tag: string) => (
      ...
      {post.image && (
        <>
          <div className="mt-4 cursor-pointer" onClick={() => setIsImageModalOpen(true)}>
            <img
              src={post.image}
              alt="Post"
            ...
            <div className="relative">
              ...
              {showShareMenu && (
                <div className="absolute z-20 mt-2 w-56 right-0 bg-white border border-gray-200 rounded-lg shadow-lg p-3 flex flex-col gap-2">
                  <button onClick={handleShare} className="flex items-center gap-2 text-gray-700 hover:text-primary-600">
                    <FiShare className="w-4 h-4" />
                    <span>Copy link / Native share</span>
                  </button>
                  <a href={twitterUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-blue-500 hover:underline">
                  ...
                  <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-green-600 hover:underline">
                    <svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M20.52 3.48A11.93 11.93 0 0 0 12 0C5.37 0 0 5.37 0 12c0 2.11.55 4.16 1.6 5.97L0 24l6.18-1.62A11.94 11.94 0 0 0 12 24c6.63 0 12-5.37 12-12 0-3.19-1.24-6.19-3.48-8.52zM12 22c-1.85 0-3.68-.5-5.25-1.44l-.37-.22-3.67.97.98-3.58-.24-.37A9.94 9.94 0 0 1 2 12c0-5.52 4.48-10 10-10s10 4.48 10 10-4.48 10-10 10zm5.2-7.6c-.28-.14-1.65-.81-1.9-.9-.25-.09-.43-.14-.61.14-.18.28-.28.7.9-.86 1.08-.16.18-.32.2-.6.07-.28-.14-1.18-.44-2.25-1.41-.83-.74-1.39-1.65-1.55-1.93-.16-.28-.02-.43.12-.57.13-.13.28-.34.42-.51.14-.17.18-.29.28-.48.09-.19.05-.36-.02-.5-.07-.14-.61-1.48-.84-2.03-.22-.53-.45-.46-.62-.47-.16-.01-.35-.01-.54-.01-.19 0-.5.07-.76.34-.26.27-1 1-.97 2.43.03 1.43 1.03 2.81 1.18 3.01.15.2 2.03 3.1 4.93 4.23.69.3 1.23.48 1.65.61.69.22 1.32.19 1.81.12.55-.08 1.65-.67 1.88-1.32.23-.65.23-1.2.16-1.32-.07-.12-.25-.19-.53-.33z"/></svg>
                    <span>Share on WhatsApp</span>
                  </a>
                  <a href={facebookUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-blue-700 hover:underline">
                  ...
                  <button onClick={() => { setShowShareMenu(false); setShowInternalShare(true); }} className="flex items-center gap-2 text-purple-600 hover:underline">
                    <FiSend className="w-4 h-4" />
                    <span>Share in message</span>
                  </button>
                  <button onClick={() => setShowShareMenu(false)} className="flex items-center gap-2 text-gray-400 hover:text-gray-600 mt-2">
          ...
          {user && (
            <form onSubmit={handleCommentSubmit} className="flex items-center space-x-2 mt-2">
              <input
                ...
                autoComplete="off"
              />
              <Button type="submit" disabled={submitting || !commentInput.trim()} size="sm">
                ...
                <div key={comment._id} className={`flex items-start space-x-2 group transition-opacity duration-200 ${deletingCommentId === comment._id ? 'opacity-50' : 'opacity-100'}`}> 
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-gray-900 text-xs">{comment.author?.firstName} {comment.author?.lastName}</span>
                      <span className="text-xs text-gray-400">· {comment.createdAt ? formatDistanceToNow(new Date(comment.createdAt), { addSuffix: true }) : ''}</span>
      ...
      {showInternalShare && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
            <button onClick={() => setShowInternalShare(false)} className="absolute top-2 right-2 text-gray-400 hover:text-gray-600"><FiX className="w-5 h-5" /></button>
                ...
                <div key={u._id} className="flex items-center justify-between py-2 border-b last:border-b-0">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold">
                      {u.avatar ? <img src={u.avatar} alt="" className="w-8 h-8 rounded-full object-cover" /> : `${u.firstName[0]}${u.lastName[0]}`}
                    </div>
